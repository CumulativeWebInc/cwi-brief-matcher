# OS Retrofit — Brief Matcher (slot 15)
...
- OS fit: nervous-system project state `os-retrofit-15-brief-matcher` with evidence-graded claims; 21-gate theorem verdict recorded.

## Red-team pass (2026-09-23)
- ?brief= is reflected into the DOM only through esc() — no XSS.
- Scoring quirk (shipped behavior, now pinned by tests): a token that is both an exact tag and a SYNONYMS key scores tag(3)+synonym(2) (e.g. 'dark' = 5). Layers stack by design; page copy documents per-layer weights.
- No test suite shipped with the repo (registry claimed 15/15); added tests/test.js (15 assertions) — the claim is now true.
...
## Tests
node --test tests/test.js — 15/15 (suite added in this retrofit; repo previously shipped none)
